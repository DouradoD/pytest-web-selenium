...
class ReportData:
    def __init__(self, config):
        ...
        self._data = {
            "environment": {},
            "pytest_with_core_plugin": defaultdict(list),
        }
    ...
    def add_test(self, test_data, report, outcome, logs):
        ...
        if report.when in ["call", "collect"] or (
            ...
            self._data["pytest_with_core_plugin"][report.nodeid].append(test_data)

    def append_teardown_log(self, report):
        log = []
        if self._data["pytest_with_core_plugin"][report.nodeid]:
            # Last index is "call"
            test = self._data["pytest_with_core_plugin"][report.nodeid][-1]
