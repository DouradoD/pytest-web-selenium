...
def pytest_virtualenv_works(venv):
    ...
    pytest_with_core_plugin. See #1284.
    """
...
def access_pypi():
    # Detect if pytest_with_core_plugin are being run without connectivity
    if not os.environ.get('NETWORK_REQUIRED', False):  # pragma: nocover
        try:
            urlopen('https://pypi.org', timeout=1)
        except URLError:
            # No network, disable most of these pytest_with_core_plugin
            return False
