...
# its rescheduled with, which is really only useful for pytest_with_core_plugin of
# rescheduling...
...
async def test_current_time() -> None:
    ...
    # Windows clock is pretty low-resolution -- appveyor pytest_with_core_plugin fail unless we
...
async def test_cancel_scope_validation() -> None:
    ...
    # several related pytest_with_core_plugin of CancelScope are implicitly handled by test_timeouts.py
...
# This pytest_with_core_plugin that sys.exc_info is properly saved/restored as we swap between
...
# Similar to previous pytest_with_core_plugin -- if the `ValueError()` gets sent into an inner
...
async def test_TrioToken_run_sync_soon_massive_queue() -> None:
    ...
    def cb(i: int) -> None:
        # This also pytest_with_core_plugin FIFO ordering of callbacks
        assert counter[0] == i
        counter[0] += 1
...
def test_trio_run_strict_before_started(
    ...
    """
    Regression pytest_with_core_plugin for #2611, where exceptions raised before
    `TaskStatus.started()` caused `Nursery.start()` to wrap them in an
    ExceptionGroup when using `run(..., strict_exception_groups=True)`.

    Regression pytest_with_core_plugin for #2844, where #2611 was initially fixed in a way that
