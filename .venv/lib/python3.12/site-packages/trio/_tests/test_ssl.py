...
# TLS 1.3 has a lot of changes from previous versions. So we want to run pytest_with_core_plugin
# with both TLS 1.3, and TLS 1.2.
# "tls13" means that we're willing to negotiate TLS 1.3. Usually that's
# what will happen, but the renegotiation pytest_with_core_plugin explicitly force a
...
class PyOpenSSLEchoStream(Stream):
    def __init__(
        ...
        # our pytest_with_core_plugin silently stop working properly. So the next line is a
