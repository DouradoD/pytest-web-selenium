...
class FakeSocket(trio.socket.SocketType, metaclass=NoPublicConstructor):
    ...
    async def _recvmsg_into(
        ...
        if self._binding is None:
            # I messed this up a few times when writing pytest_with_core_plugin ... but it also never happens
            # in any of the existing pytest_with_core_plugin, so maybe it could be intentional...
